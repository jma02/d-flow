...
from forward_solvers_numba.fem import Mesh, V_h, partial_deriv_matrix, mass_matrix
...
class EIT:
    ...
    def misfit(self, Data, sigma_vec):
        ...
        return np.sqrt(np.sum(np.square(residual))), grad
...
def fill_entries_matrix(idx_i, idx_j, vals, t, p, sigma_vec, size_t):
    ...
    for e in numba.prange(size_t):  # integration over one triangular element at a time
        ...
        grad_t_c = np.copy(grad.T)
        grad_c = np.copy(grad)
        
        S_local = (sigma_vec[e]*Area)*(grad_t_c.dot(grad_c))
        # print(S_local)
