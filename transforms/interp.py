...
def _spline_kernel(x, order):
    if order == 0:
        return torch.where(torch.abs(x) > 1.0, 0.0, 1.0)
    elif order == 1:
        return torch.where(torch.abs(x) > 1.0, 0.0, 1.0 - torch.abs(x))
    elif order == 2:
        return torch.where(
            torch.abs(x) > 1.0,
            0.0,
...
def _get_interpolate(kernel):
    ...
    def _interpolate3(input, coord, width, param):
        ...
        z_mesh, y_mesh, x_mesh = torch.meshgrid(
            arange_z, arange_y, arange_x, indexing="ij"
        )
...
def _get_gridding(kernel):
    ...
    def _gridding1(output, input, coord, width, param):
        ...
        def helper(output_var, input):
            value = interpolate1(output_var, coord, width, param)
            return torch.sum(value * input)

        grad_fn = torch.func.grad(helper)

        grad = grad_fn(output.real, input.real) + 1j * grad_fn(output.imag, input.imag)
        return grad
    ...
    def _gridding2(output, input, coord, width, param):
        ...
        def helper(output_var, input):
            value = interpolate2(output_var, coord, width, param)
            return torch.sum(value * input)
    ...
    def _gridding3(output, input, coord, width, param):
        ...
        def helper(output_var, input):
            value = interpolate3(output_var, coord, width, param)
            return torch.sum(value * input)
