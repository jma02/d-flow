...
from scripts.transforms_jax import util
...
def _spline_kernel(x, order):
    if order == 0:
        return jnp.where(jnp.abs(x) > 1.0, 0.0, 1.0)
    elif order == 1:
        return jnp.where(jnp.abs(x) > 1.0, 0.0, 1.0 - jnp.abs(x))
    elif order == 2:
        return jnp.where(
            jnp.abs(x) > 1.0,
            0.0,
...
def _get_interpolate(kernel):
    ...
    def _interpolate3(input, coord, width, param):
        ...
        z_mesh, y_mesh, x_mesh = jnp.meshgrid(
