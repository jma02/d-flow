...
from scripts.transforms_jax import fourier, util
import math
...
def get_kspace_radial(diameter, expanded_diameter, n_projections):
    ...
    a = np.linspace(0, np.pi, n_projections)
    r_grid, a_grid = np.meshgrid(r, a, indexing="xy")
    x = (
        np.round((r_grid * np.cos(a_grid)) * expanded_diameter / diameter)
        % expanded_diameter
    )
    y = (
        np.round((-r_grid * np.sin(a_grid)) * expanded_diameter / diameter)
        % expanded_diameter
    )
...
def radon_transform(image, N=50):
    ...
    kspace = fourier.nufft(
        image, jnp.stack([y, x], axis=-1), oversamp=oversamp, width=width
    )
    sinogram = (
...
def fft_radon_transform(image, N=50, expansion=6):
    ...
    sinogram = jnp.fft.fftshift(
        jnp.fft.ifft(jnp.fft.ifftshift(slices, axes=-1), axis=-1), axes=-1
    )
...
def fft_sino_to_kspace(sino, n_projections, size, expansion):
    ...
    slices = jnp.fft.fftshift(
        jnp.fft.fft(jnp.fft.ifftshift(sino, axes=-1), axis=-1), axes=-1
    )
    oshape = sino.shape[:-2] + (expanded_diameter, expanded_diameter)
    kspace = (
        jnp.zeros(oshape, dtype=jnp.complex64)
        .at[..., y.astype(jnp.int32), x.astype(jnp.int32)]
        .set(slices)
    )
    return kspace
...
def fft_discretize_sinogram(image, sino, expansion=8):
    ...
    round_y = (
        np.round(circle_y * expanded_diameter / diameter) * diameter / expanded_diameter
    )
    round_x = (
        np.round(circle_x * expanded_diameter / diameter) * diameter / expanded_diameter
    )
    ...
    def interp_slices(slices):
        slices = jax.scipy.ndimage.map_coordinates(
            slices, np.stack([round_theta, round_r], axis=0), order=1, mode="nearest"
        )
...
def get_fourier_filter(diameter, K, oversamp=1.25, width=4):
    ...
    fourier_filter = (
        2
        * fourier.nufft(
            jnp.fft.fftshift(f), r[:, None], oversamp=oversamp, width=width
        ).squeeze()
        * np.sqrt(size)
    )
...
def iradon_transform(sinogram, K=1.8):
    ...
    kspace = fourier.nufft(
        sinogram, r[:, None], oversamp=oversamp, width=width
    ) * np.sqrt(diameter)
    image = (
        fourier.nufft_adjoint(
            kspace * fourier_filter,
            jnp.stack([y, x], axis=-1),
