...
def get_loaders_interpolant(config):
    ...
    dataset = torch.load(f'data/{problem}-sparse-and-full-{n_sub}-{n_full}-{image_size}.pt')
...
def get_loaders_multiflow_v1(config):
    ...
    train = TensorDataset(train_sub.detach().clone(), train_full.detach().clone(), train_media.detach().clone())
    test = TensorDataset(val_sub.detach().clone(), val_full.detach().clone(), val_media.detach().clone())
